#!/bin/bash
set -e
echo "🚀 Titanium automata setup - Termux"
...
# --- Python csomagok ---
pip3 install --upgrade google-auth google-auth-oauthlib google-auth-httplib2 google-api-python-client qrcode termcolor requests

# --- .titanium_env létrehozása ---
ENV_FILE="$HOME/.titanium_env"
cat << 'EOF' > $ENV_FILE
export NGROK_API_KEY="3AOe5EclFMhWcMnQ9GHwd1kKDGE_2s7PN4DQAreGdgkbzSipR"
export YOUTUBE_TOKEN="$HOME/youtube_token.pkl"
...
# --- Betöltés ---
source $ENV_FILE
echo "✅ Környezet betöltve. Használat: source ~/.titanium_env"

# --- YouTube OAuth (headless Termux) ---
cat << 'EOF' > youtube_oauth.py
import pickle
...
echo "📌 YouTube hitelesítés: python3 youtube_oauth.py"
