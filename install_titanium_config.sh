...
pip install --user stripe requests
...
# 5️⃣ Konfigurációs fájl létrehozása
CONFIG_FILE=~/titanium_v11/config.env

if [ ! -f "$CONFIG_FILE" ]; then
  echo "💡 Kérlek add meg a szükséges kulcsokat:"
  read -p "Telegram bot token: " TELEGRAM_BOT_TOKEN
  read -p "Telegram chat ID: " TELEGRAM_CHAT_ID
  read -p "Stripe secret key: " STRIPE_SECRET_KEY
  read -p "Revolut API token: " REVOLUT_API_TOKEN
  read -p "Revolut payout threshold (€) (alap: 50): " REVOLUT_PAYOUT_THRESHOLD
  REVOLUT_PAYOUT_THRESHOLD=${REVOLUT_PAYOUT_THRESHOLD:-50}

  cat > "$CONFIG_FILE" <<EOF
TELEGRAM_BOT_TOKEN="$TELEGRAM_BOT_TOKEN"
TELEGRAM_CHAT_ID="$TELEGRAM_CHAT_ID"
STRIPE_SECRET_KEY="$STRIPE_SECRET_KEY"
REVOLUT_API_TOKEN="$REVOLUT_API_TOKEN"
REVOLUT_PAYOUT_THRESHOLD="$REVOLUT_PAYOUT_THRESHOLD"
EOF

  echo "✅ Konfiguráció elmentve: $CONFIG_FILE"
else
  echo "ℹ️ Konfigurációs fájl már létezik, használatban: $CONFIG_FILE"
fi

# 6️⃣ AI indító script
...
# 7️⃣ Revenue monitor script
cat > ~/titanium_v11/revenue_monitor.py << 'EOF'
import os
import time
import requests

# Betöltjük a konfigurációs fájlt
CONFIG_FILE = os.path.join(os.getenv("HOME"), "titanium_v11", "config.env")
...
REVOLUT_THRESHOLD = float(os.getenv("REVOLUT_PAYOUT_THRESHOLD", "50"))
...
# 9️⃣ Termux boot automatikus indítás
mkdir -p ~/.termux/boot
